...
# Page configuration
st.set_page_config(page_title="Singapore Employment Dashboard", layout="wide", initial_sidebar_state="expanded")
...
with col2:
    if st.button("Reset", key="reset_years", help="Reset to full range"):
...
# Main title
st.title("Singapore Employment Analytics Dashboard")
st.markdown("### Comprehensive Analysis of Employment Trends, Income, and Job Markets")
st.markdown("---")
...
with tab1:
    ...
    with col1:
        ...
        try:
            ...
            st.error(f"Error loading Seif's data: {e}")
    ...
    with col1:
        try:
            ...
            st.error(f"Error loading Delwin's data (Chart 1): {e}")
    ...
    with col2:
        try:
            ...
            st.error(f"Error loading Delwin's data (Chart 2): {e}")
    ...
    with col1:
        try:
            ...
            st.error(f"Error loading Raghav's data (Chart 1): {e}")
    ...
    with col2:
        try:
            ...
            st.error(f"Error loading Raghav's data (Chart 2): {e}")
    ...
    with col1:
        try:
            ...
            st.error(f"Error loading Vedika's data (Chart 1): {e}")
    ...
    with col2:
        try:
            ...
            fig.update_xaxes(side="top", dtick=1)  # Fix year axis to show only integers
            fig.update_layout(height=500)
            st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
            st.error(f"Error loading Vedika's data (Chart 2): {e}")
...
with tab2:
    ...
    try:
        ...
        st.error(f"Error loading Timothy's data: {e}")
    ...
    try:
        ...
        st.error(f"Error loading Nabodita's data: {e}")

# ==================== AI CHATBOT SECTION ====================
st.markdown("---")
st.header("🤖 AI Employment Data Assistant")
st.markdown("Ask questions about Singapore employment trends, income, vacancies, and more!")
...
try:
    ...
    def generate_data_context():
        """Generate a summary of available data for AI context"""
        try:
            kpi_info = f"""
        Current KPI values (latest year {year_range[1]}):
        - Total Employment: {total_employment_current:.1f}K (YoY: {employment_growth:+.1f}%)
        - Median Income: ${avg_median_income:,.0f} (YoY: {income_change:+.1f}%)
        - Job Vacancies: {total_vacancies:,.0f} (YoY: {vacancies_change:+.1f}%)
        - Male Employment: {male_pct:.1f}%
        - Female Employment: {female_pct:.1f}%
        - Avg Work Hours: {avg_hours:.1f} hrs/week
        - Higher Education: {higher_ed_pct:.1f}%
        """
        except:
            kpi_info = "\nKPI values are available in the dashboard above."

        context = f"""
        You are an AI assistant helping users understand Singapore employment data from 2015-2024.

        Current filters applied:
        - Year Range: {year_range[0]} to {year_range[1]}
        - Selected Industries: {', '.join(selected_industries) if selected_industries else 'None selected'}

        Available data categories:
        1. Employment by Industry (M182081.csv) - Total employed residents across 12 industries
        2. Gender Employment (M182141.csv, M182151.csv) - Male and female employment breakdown
        3. Education Levels (M182271.csv) - Labour force by education (Degree, Diploma, Secondary, etc.)
        4. Working Hours (M184061.csv, M184091.csv) - Average hours worked and usual hours by industry
        5. Job Vacancies (M184071.csv) - Available positions by industry
        6. Income (FT_Res_ind_income.csv) - Median gross monthly income by industry
        7. Bonuses (mrsd_18_Variable_component.csv) - Annual variable component by industry

        Industries covered:
        {', '.join(Industries)}
        {kpi_info}

        Answer questions based on this data. Be specific, use numbers when relevant, and provide insights about trends.
        If asked about data outside the scope, politely indicate what data is available.
        Keep responses concise and focused on the Singapore employment context.
        """
        return context
    ...
    with chat_col1:
        ...
        # User input
        user_input = st.chat_input("Ask a question about Singapore employment data...")
        ...
        if user_input:
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    messages = [
                        {"role": "system", "content": generate_data_context()},
                    ]
    ...
    with chat_col2:
        ...
        st.markdown("---")
        st.markdown("**Tips:**")
        st.markdown("- Ask about employment trends")
        st.markdown("- Compare industries")
        st.markdown("- Inquire about income levels")
        st.markdown("- Explore gender gaps")
        st.markdown("- Understand education impact")

        st.markdown("---")
        st.info("💡 The AI has access to all data shown in the dashboard based on your current filters.")

except KeyError:
    st.warning("⚠️ OpenAI API key not found in secrets. Please add 'OpenAI_key' to your .streamlit/secrets.toml file.")
    st.info("""
    **How to add the API key:**
    ...
    st.error(f"⚠️ Error initializing AI chatbot: {str(e)}")

# Footer
st.markdown("---")
st.markdown("**Data Sources:** Singapore Department of Statistics | **Dashboard by:** Team 10")
